...
class Core
  {
    private:
      ...
      unsigned char *replay_packet;
      int replay_packet_len;
      ...
      int recv_callback (struct nfq_q_handle *q_handle,
                    ...
                    struct nfq_data *nfa)
        {
          ...
          memcpy (timestamp, pkt + sizeof (struct ip6_hdr) + sizeof (struct udphdr) + 2, 6);
          ...
          is_vinet = this->callback_receive (payload, payload_length);
          ...
          set_verdict:
            ...
            int verdict = -1;
            if (is_vinet) {
              memcpy (this->replay_packet + sizeof (struct ip6_hdr) + sizeof (struct udphdr), (char *) (pkt + sizeof (struct ip6_hdr) + sizeof (struct udphdr)), rtp_header_size);
              struct ip6_hdr *ip_header = (struct ip6_hdr *) this->replay_packet;
              struct udphdr *udp_header = (struct udphdr *) (this->replay_packet + sizeof (struct ip6_hdr));
              this->update_checksum (ip_header, udp_header);  
              verdict = nfq_set_verdict (q_handle, packet_id, NF_ACCEPT, this->replay_packet_len, this->replay_packet);
            }

            else {
              this->replay_packet = pkt;
              this->replay_packet_len = len;
              this->update_checksum (ip_header, udp_header);
              verdict = nfq_set_verdict (q_handle, packet_id, NF_ACCEPT, len, pkt);
            }
      ...
      void callback_send (ip6_hdr *ip_header,
        ...
        bool callback_receive (char *payload,
                               int payload_length)
          {
            char data[2048];
            int data_length;
